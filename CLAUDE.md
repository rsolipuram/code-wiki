...
- **macOS fork safety**: RQ's default forking worker causes SIGABRT on macOS when native frameworks (PyTorch/SentenceTransformer for embeddings, Neo4j driver, SSL) are used in the forked child process. `OBJC_DISABLE_INITIALIZE_FORK_SAFETY=YES` alone is insufficient. **Must use SimpleWorker**: `rq worker --worker-class rq.SimpleWorker analysis` — runs jobs in the main process without forking, so all components (Neo4j, Qdrant, embeddings) work normally
